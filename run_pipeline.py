...
class Args:
    def __init__(self, videofile, data_dir):
        ...
        self.batch_size = 64 # 64 occurs CUDA out of memory
        self.vshift = 15
...
def main(args):
    ...
    time_steps = [float(df[2].iloc()[i]) for i, j in enumerate(df[1].iloc()) if j == "OK"]
    ...
    time_log = video_validation_log[:-4] + "_time" + video_validation_log[-4:]

    # initialize csv file
    with open(video_validation_log, "w") as f:
...
def process(video_path, interval, start_time, end_time, output_path, video_validation_log, s, DET, st, time_log):
    ...
    command = ("ffmpeg -y -ss %s -to %s  -i %s -vf 'transpose=2' -qscale:v 2 -async 1 -r 25 %s" % (start_time, end_time, opt.videofile, os.path.join(opt.avi_dir, opt.reference, "video.avi")))
    output = subprocess.call(command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    command = ("ffmpeg -y -ss %s -to %s -i %s -ac 1 -vn -acodec pcm_s16le -ar 16000 %s" % (start_time, end_time, opt.videofile,os.path.join(opt.avi_dir,opt.reference,'audio.wav')))
    output = subprocess.call(command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    # ========== FACE DETECTION ==========
